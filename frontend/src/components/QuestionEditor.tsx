...
export function QuestionEditor({
  ...
  const updateQ = (id: string, patch: Partial<EnhancedQuestion>) => {
    ...
    const next = questions.map(q => {
      ...
      return q; // Return unchanged question for all others
    });
  ...
  // Enhanced single question enhancement - only update the specific question
  const enhanceQuestion = async (questionIndex: number) => {
    ...
    try {
      ...
      // Only update the SPECIFIC question being enhanced
            ...
            <div key={q.id} className={`border-t pt-6 first:border-t-0 first:pt-0 transition-all duration-300 ${
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  {/* Individual Enhancement Button - Allow re-enhancement when no other enhancement is running */}
